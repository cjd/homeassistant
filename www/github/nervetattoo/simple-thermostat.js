...
`;function at(t,{decimals:e=1,fallback:s="N/A"}={}){if(null===t||""===t||["boolean","undefined"].includes(typeof t))return s;const[i,n]=String(t).split(".");return Number.isNaN(i)?s:e?`${i}.${n||"0"}`:String(Math.round(t))}const lt="dual",ht="single";const ct=1e3,dt=.5,pt=1,ut=["entity","sensors","_values"],mt={auto:"hass:autorenew",manual:"hass:cursor-pointer",heat:"hass:fire",cool:"hass:snowflake",off:"hass:power",fan_only:"hass:fan",fan:"hass:fan",eco:"hass:leaf",dry:"hass:water-percent",idle:"hass:power"},ft={off:"mdi:radiator-off",on:"mdi:radiator",idle:"mdi:radiator-disabled",heat:"mdi:radiator",cool:"mdi:snowflake",auto:"mdi:radiator",manual:"mdi:radiator",boost:"mdi:fire",away:"mdi:radiator-disabled"},_t={temperature:!1,state:!1,mode:!1,away:!0};class gt extends et{static get styles(){return ot}static get properties(){return{_hass:Object,config:Object,entity:Object,sensors:Array,modes:Object,icon:String,_values:Object,_mode:String,_hide:Object,name:String}}constructor(){super(),this._debouncedSetTemperature=rt(()=>{this._hass.callService("climate","set_temperature",{entity_id:this.config.entity,...this._values})},{wait:ct}),this._hass=null,this.entity=null,this.icon=null,this.sensors=[],this._stepSize=dt,this._values={},this._mode=null,this._hide=_t,this._haVersion=null}set hass(t){this._hass=t;const e=t.states[this.config.entity];if(void 0===e)return;this._haVersion=t.config.version.split(".").map(t=>parseInt(t,10));const{attributes:{operation_mode:s,operation_list:i=[],...n}}=e;if(this.entityType=function(t){return"number"==typeof t.target_temp_high&&"number"==typeof t.target_temp_low?lt:ht}(n),"dual"===this.entityType?this._values={target_temp_low:n.target_temp_low,target_temp_high:n.target_temp_high}:this._values={temperature:n.temperature},this.entity!==e){this.entity=e,this._mode=s,this.modes={};for(const t of i)this.modes[t]={include:!0,icon:mt[t]}}!1===this.config.modes?this.modes=!1:"object"==typeof this.config.modes&&Object.entries(this.config.modes).map(([t,e])=>{this.modes.hasOwnProperty(t)&&("boolean"==typeof e?this.modes[t].include=e:this.modes[t]={...this.modes[t],...e})}),this.config.icon?this.icon=this.config.icon:this.icon=ft,this.config.step_size&&(this._stepSize=this.config.step_size),this.config.hide&&(this._hide={...this._hide,...this.config.hide}),"string"==typeof this.config.name?this.name=this.config.name:!1===this.config.name?this.name=!1:this.name=e.attributes.friendly_name,this.config.sensors&&(this.sensors=this.config.sensors.map(({name:e,entity:s,attribute:i,unit:n=""})=>{let r;const o=[e];return s?(r=t.states[s],o.push(r&&r.attributes&&r.attributes.friendly_name)):i&&i in this.entity.attributes&&(r=this.entity.attributes[i]+n,o.push(i)),o.push(s),{name:o.find(t=>!!t),state:r,entity:s}}))}shouldUpdate(t){return ut.some(e=>t.has(e))}localize(t,e=""){const s=this._hass.selectedLanguage||this._hass.language,i=`${e}${t}`,n=this._hass.resources[s];return i in n?n[i]:t}render({_hass:t,_hide:e,_values:s,config:i,entity:n,sensors:r}=this){if(!n)return $`
    ...
    `}renderModeSelector(t){if(!1===this.modes)return;const e=Object.entries(this.modes).filter(([t,e])=>e.include),s=(t,e)=>!1===e.name?null:e.name||this.localize(t,"state.climate."),i=t=>!1===t?null:$`
        <ha-icon class="mode__icon" .icon=${t}></ha-icon>
      `;return this._haVersion[1]<=87?$`
        <div class="modes">
          ${e.map(([e,n])=>$`
              <paper-button
